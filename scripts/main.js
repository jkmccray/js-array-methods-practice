...
// Array to contain all the big spenders (businesses with any orders over $9000)
...
const capitalizedPlanets = planets.map(planet => {
  const capFirstLetter = planet.charAt(0).toUpperCase()
  const restOfLetters = planet.slice(1)
  return capFirstLetter + restOfLetters
})
...
const planetsWithOnlyE = planets.filter(planet => {
